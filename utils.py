...
def fig_hist(params, loss_hist, metric_hist, config=None):
    ...
    sns.lineplot(x=[*range(1, epochs + 1)], y=loss_hist["train"], ax=ax, label='loss_hist["train"]')
    sns.lineplot(x=[*range(1, epochs + 1)], y=loss_hist["val"], ax=ax, label='loss_hist["val"]')
